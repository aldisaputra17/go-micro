...
func init() {
	...
	defaultLogger = NewLogger(constant.LogLevelDebug, nil, nil)
}
...
// Error logs error messages.
func (l *Logger) NewError(err error, msg string, meta ...interface{}) error {
	l.printLogFmt()
	l.errorf(err, msg, meta)
	l.printLogFmt()
...
func (l *Logger) debugf(message string, fields []interface{}) {
	if l.Level > constant.LogLevelDebug {
		return
	}
...
func (l *Logger) infof(message string, fields []interface{}) {
	if l.Level > constant.LogLevelInfo {
		return
	}
...
func (l *Logger) warnf(message string, fields []interface{}) {
	if l.Level > constant.LogLevelWarn {
		return
	}
...
func (l *Logger) UpdateLogLevel(level Level) {
	...
	l.Level = constant.LogLevelError
	if level < constant.LogLevelDisabled || level > constant.LogLevelError {
		l.Level = level
...
func setLogLevel(l *zerolog.Logger, level Level) {
	switch level {
	case constant.LogLevelDisabled:
...
func Print(v ...interface{}) {
	if fileFmt, ok := fileLineLogFmt(constant.LogLevelInfo); ok {
		stdLog.Print(fileFmt + fmt.Sprint(v...))
		return
	}

	_ = stdLog.Output(constant.LogDefaultStdLogSkipCallerCount, fmt.Sprint(v...))
}

// Printf calls Output to print to the standard logger.
// Arguments are handled in the manner of fmt.Printf.
func Printf(format string, v ...interface{}) {
	if fileFmt, ok := stdFileLineLogFmt(constant.LogLevelInfo); ok {
		stdLog.Printf(fileFmt+format, v...)
		return
	}

	_ = stdLog.Output(constant.LogDefaultStdLogSkipCallerCount, fmt.Sprintf(format+"\n", v...))
}
...
func Println(v ...interface{}) {
	...
	_ = stdLog.Output(constant.LogDefaultStdLogSkipCallerCount, fmt.Sprintln(v...))
}

// Fatal is equivalent to Print() followed by a call to os.Exit(1).
func Fatal(v ...interface{}) {
	if fileFmt, ok := stdFileLineLogFmt(constant.LogLevelError); ok {
